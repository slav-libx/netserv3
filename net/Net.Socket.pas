...

{
    ...
    begin
      ...
      var
        ...
        TCPSockets: array of array of TTCPSocket;
      begin
        ...
        begin
          ...
          if List[I].Connected then
          begin
            if C mod 64=0 then Sockets:=Sockets+[];
//            Sockets[C div 64]:=Sockets[C div 64]+[List[I].Socket];
//            TCPSockets:=TCPSockets+[List[I]];
            Inc(C);
          end;
          TMonitor.Exit(Self);

          for C:=0 to High(Sockets) do
          begin

          ReadFds:=TFDSet.Create(Sockets[C]);

          if Length(Sockets)>0 then
          try

            if TSocket.Select(ReadFds, nil, nil, 10*1000)=wrSignaled then
            for Socket in TCPSockets do
            try
            if ReadFds.IsSet(Socket.Socket) then
            case Socket.Socket.ReceiveLength of
           -1: beep;
            0:
...
//    if Lock.Terminated then Exit;
//
//    DoLog('TTCPSocket.DoConnect('+AName+') Receive loop is terminated');
//
//    if Connected then
//
//    TThread.Synchronize(SyncThread,
